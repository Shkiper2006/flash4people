...
function connectWebSocket(authPayload, isRetry = false) {
  if (state.ws) {
    state.ws.close();
  }
  if (!isRetry) {
  ...
  if (authPayload) {
    state.lastAuthPayload = authPayload;
  }

  const wsUrl = state.wsUrlOverride || getDefaultWsUrl();
  ...
  ws.addEventListener('open', () => {
    ...
    if (authPayload?.token) {
      sendWs({ type: 'auth_token', token: authPayload.token });
    } else if (authPayload?.mode) {
  ...
  ws.addEventListener('close', () => {
    ...
    if (state.hasAuthAttempted && !state.isAuthed) {
      scheduleWsReconnect();
    }
    render();
  });
...
function scheduleWsReconnect() {
  ...
  if (state.wsRetryCount >= 3) {
    setError('Не удалось подключиться к WebSocket. Проверьте, что сервер запущен и порт открыт.');
    return;
  }
  const delay = 1000 * (state.wsRetryCount + 1);
  state.wsRetryCount += 1;
  if (!state.wsUrlOverride && window.location.port !== '3001') {
    const protocol = window.location.protocol === 'https:' ? 'wss' : 'ws';
    state.wsUrlOverride = `${protocol}://${window.location.hostname}:3001/ws`;
  }
  state.wsRetryTimer = setTimeout(() => {
    state.wsRetryTimer = null;
    connectWebSocket(state.lastAuthPayload, true);
  }, delay);
}
